...
### Hot Fluid
Temperature drop:
$\Delta T_h := \frac{Q}{m_h \cdot c_p} ==$

Outlet temperature:
$T_{h,out} := T_{h,in} - \Delta T_h ==$

### Cold Fluid
Temperature rise:
$\Delta T_c := \frac{Q}{m_c \cdot c_p} ==$

Outlet temperature:
$T_{c,out} := T_{c,in} + \Delta T_c ==$
...
$ratio := \frac{\Delta T_1}{\Delta T_2} ==$

$$LMTD := \frac{\Delta T_1 - \Delta T_2}{\ln(ratio)} ==$$
...
$Qmax := Cmin \cdot (T_{h,in} - T_{c,in}) ==$
