...
<objective>
Fix ISSUE-005: LaTeX-wrapped units (`\text{m/s}^2`) should be parsed correctly by Pint.
...
<task type="auto">
  ...
  <action>
Add new test class to `tests/test_pint_backend.py` for ISSUE-005:
...
def clean_latex_unit(latex_unit: str) -> str:
    ...
    unit = unit.replace('·', '*')  # Unicode middle dot
...
Key principle: LaTeX → clean_latex_unit() → Pint
...
4. Update `examples/README.md` if needed to document LaTeX unit support
  ...
  <done>All tests pass, BACKLOG.md updated with ISSUE-005 resolution</done>
</task>
...
- [ ] ISSUE-005 marked resolved in BACKLOG.md
...
- Phase 2 complete (both ISSUE-004 and ISSUE-005 resolved)
</success_criteria>
